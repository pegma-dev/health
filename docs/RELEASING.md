...
`@pegma/health` publishes only from a stable GitHub release. Merging a pull
...
- configure `@pegma/health` on npm with the GitHub Actions trusted publisher
  `pegma-dev/health`, workflow `publish.yml`, environment `npm-publish`, and
...
The version in `packages/health/package.json` is the release version. Change it
